...
The Open Access Directory has a worldwide list of [university
...
library(htmltools)
library(DT)
cities = read_csv("places.csv",comment="#")
rr     = read_csv("ukinstitutions.csv", comment = "#")
merged <- merge(x=rr, y=cities, by="Institution", all=TRUE)
merged <- merged %>% mutate(label = sprintf("<a href=\"%s\">%s</a>",
                                            Reference, Institution))
stopifnot(! any(is.na(merged$lat)))  # ALL join did not work if we have NAs.
...
Liverpool School of Tropical Medicine has a policy live from 2025-01-01; the
link for it will be online in due course.
